...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('ꜱᴏᴜʀᴄᴇ ᴄᴏᴅᴇ 📜', url='https://t.me/Sandymaiwait'),
        ...
        if len(message.text) < 100:
            ...
            if not files:
                if settings.get("spell_check", False):
                    ...
                    try:
                        # Keep the existing IMDb-based spelling checker, but
                        # protect it with a timeout so a slow IMDb request can
                        # never leave the user stuck on the waiting message.
                        ...
                        LOGGER.warning(f"AI spelling check failed/timeout: {e}")
                        is_misspelled = None

                    if is_misspelled:
                        await ai_sts.edit(
                            f'<b><i>✅ Aɪ Sᴜɢɢᴇsᴛᴇᴅ ᴍᴇ <code>{is_misspelled}</code>\n'
                            f'Sᴏ Iᴍ SᴇᴀRᴄʜɪɴɢ ғᴏʀ <code>{is_misspelled}</code></i></b>'
                        )
                ...
                # No exact DB match and no verified AI correction. Only now
                # show the final search-help/no-result message.
...
async def ai_spell_check(chat_id, wrong_name):
    ...
    async def imdb_titles(q):
        try:
            ...
            LOGGER.warning(f"IMDb spelling search failed for {q!r}: {e}")
            return []

    # IMDb may not correct a heavily misspelled string in one request, so try
    # the collapsed-letter form as well. This is particularly useful for
    # dhuranndhar -> dhurandhar and repeated-letter phone typing mistakes.
    for q in queries:

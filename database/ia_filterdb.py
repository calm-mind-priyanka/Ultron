...
async def get_fuzzy_search_results(chat_id, query: str, limit: int = 25, threshold: int = 82):
    ...
    if MULTIPLE_DB:
        primary, secondary = await asyncio.gather(fetch(Media), fetch(Media2))
    else:
    ...
    for item in primary + secondary:
        ...
        # Two strong scorers are enough here. Keeping this local calculation
        # small is important because this function is only a fallback.
        score = max(
